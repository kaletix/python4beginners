...
def ewakuacja(lista_osob, liczba_klatek_schodowych, liczba_osob_w_rzedzie, tempo_schodzenia):
    floors=len(lista_osob)
    t=[0]*floors
    max_osob=[]
    for i in range(floors):
        max_osob.append(int(lista_osob[i]/liczba_klatek_schodowych)+lista_osob[i]%liczba_klatek_schodowych)
    # print(max_osob)

    for i in range(len(max_osob)-1):
        t[-1]=0
        t[-(i+2)]=t[-(i+1)]+tempo_schodzenia+int(max_osob[-(i+1)]/liczba_osob_w_rzedzie)+max_osob[-(i+1)]%liczba_osob_w_rzedzie
