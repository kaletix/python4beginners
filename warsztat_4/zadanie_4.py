...
# wiem, że już za późno:) ale udało mi się jeszcze zrobić czwarte
...
class Picture:
    ...
    def red(self):
        red_tuple = tuple(self.r)
        return red_tuple

    def green(self):
        green_tuple = tuple(self.g)
        return green_tuple

    def blue(self):
        blue_tuple = tuple(self.b)
        return blue_tuple

    def size(self):
        size_tuple = tuple()
        size_tuple += self.w + self.h
        return size_tuple
    ...
    def crop(self, x, y, width, height):
        ...
        for i in range(1, width+1):
            for j in range(height):
                new_red += self.r[index_pocz*i + j]
        img = Picture(new_red, new_green, new_blue, width, height)
        return img

    def pixel(self, x, y):
        pixel_tuple = tuple()
        nr_px = y * self.w + x
        pixel_tuple += self.r[nr_px] + self.g[nr_px] + self.b[nr_px]
        return pixel_tuple
